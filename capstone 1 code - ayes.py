...
def login(): #mendefinisikan fungsi login
    while True: #melakukan proses login sampai login berhasil
        ...
        print("Apa yang ingin ada lakukan?")
        print("1. Login sebagai admin")
        print("2. Login sebagai peminjam")
        print("3. Logout")
        pilihan = input("Pilih pengguna: ") #pilih input untuk login
...
def cariBuku(kategori, kataKunci): #mendefinisikan fungsi cari buku, kategori berisi mau cari buku berdasarkan apa, kata kunci untuk mempermudah pencarian buku
    ...
    for key, value in koleksiBuku.items(): #cek apakah setiap buku dalam koleksiBuku untuk menentukan apakah sesuai kriteria pencarian atau tidak
        ketersediaan = "Sedang Dipinjam" if key in koleksiBuku else "Tidak Tersedia"
...
#meminjam buku
def pinjamBuku(judulBuku, tanggalPinjam, namaPeminjam): #mendefinisikan fungsi pinjam buku
...
def kembalikanBuku(judulBuku, tanggalKembali, namaPengembali):
    ...
    terlambat = (tanggalKembali-tanggalPinjam)
    terlambatHari = terlambat.days - 7
    denda = max(0, terlambatHari * 10000) #denda keterlambatan 10.000 per hari
    if denda > 0:
        while True:
            print(f"Denda keterlambatan : Rp{denda}")
            jumlahUang = int(input("Masukkan jumlah uang: "))
            kembalian = jumlahUang - denda
            if jumlahUang == denda:
                print("Terima kasih telah membayar buku dan mengembalikan buku.")
                break
            elif jumlahUang < denda:
                print("Uang yang anda masukkan kurang. Silahkan masukan jumlah uang yang cukup.")
            else:
                kembalian
                print(f"Terima kasih telah membayar denda dan mengembalikan buku. Kembalian anda: Rp{kembalian}.")
                break
...
def tambahBuku():
    ...
    if any(buku["title"].lower() == judul.lower() for buku in koleksiBuku.values()):
        print("\nBuku sudah ada dalam koleksi. Tidak bisa menambahkan duplikasi.")
        return
    #membuat ID unik untuk setiap buku yang baru dimasukkan
    kodeGenre = genre[:3].upper()
    nomorBuku = sum(1 for idBuku in koleksiBuku if idBuku.startswith(kodeGenre)) + 1
    idBaru = f"{kodeGenre}{nomorBuku:03d}"

    #nambahin ke koleksi
    koleksiBuku[idBaru] = {"title": judul, "author": penulis, "genre": genre, "year": tahun}
    print(f"\nBuku '{judul}' berhasil ditambahkan dengan ID {idBaru}.")
...
def menuAdmin():
    while True:
        ...
        if pilihan == "1":
            ...
            pinjamBuku(judulBuku, tanggalPinjam, namaPeminjam)
        elif pilihan == "3":
            judulBuku = input("Masukkan judul buku: ").title()
